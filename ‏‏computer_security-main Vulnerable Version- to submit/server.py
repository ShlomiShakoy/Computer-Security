...
def add_user():
    if request.method == 'POST':
        ...
        try:
            ...
            user_exists = cursor.fetchall() #always fetch all relevant users
            #check if sqli occurred
            if len(user_exists)>1:
                timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
                log_file = f'Sqli_Data_Register_{timestamp}.log'

                # Open the log file in append mode to add each new login attempt
                with open(log_file, 'a') as f:
                    for row in user_exists:
                        username=row[0]
                        password = row[1]
                        log_message = f"username: {username}, Password: {password}\n"
                        f.write(log_message)
                cursor.close()
                conn.close()
                flash("SQL injection successful! All data exported to .log file!", 'danger')
        ...
        for username in user_exists:
            if username in user_exists: #if username already exists
...
def login():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        global global_username#declare this global variable
        conn = get_db_connection()
        cursor = conn.cursor()

        print(f"SELECT id, password FROM users WHERE username = '{username}';")
        ...
        if result:
            if len(result)>1:#if there were more than one user that retreived from database:
                timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
                log_file = f'Sqli_Data_Login_{timestamp}.log'
                ...
                with open(log_file, 'a') as f:
                    for row in result:
                        user_id = row[0]
                        password = row[1]
                        log_message = f"User ID: {user_id}, Password: {password}\n"
                        f.write(log_message)

                # Assuming you're using Flask, use this to show the message
                flash("SQL injection successful! All data exported to .log file!", 'danger')
                cursor.close()
                conn.close()
                return redirect(url_for('login'))
                ...
                user_id = result[0][0] #Getting the user_id from the database
                passwordFromDataBase = result[0][1] #Getting the password from the database
            ...
            if verify_password(passwordFromDataBase, password):
                ...
                #reset to 0 the logintries counter
                cursor.execute(
            ...
            else: #if the password was not similar, we set login tries counter
                ...
                if resultOfLoginCounter:
                    ...
                    if logincounter>=passwordRules.login_tries:#to cover cases of user goes back to login page and tries to login again. if it's set to == it will go to the else statement
                        ...
                        #login failed for more than counter times, we must reset with a default password the current password
                        default_password = os.getenv('DEFAULT_PASSWORD') #taken from .env file
                        random_number = str(random.randint(1, 100000)) #randomizing a number
...
def add_customer():
    if 'user_id' not in session: #should never arrive here
    ...
    if request.method == 'POST':
        ...
        if 'delete_all_customers' in request.form:
            try:
                #a query that deletes all customers from table for the current user
                cursor.execute(f"DELETE FROM customers WHERE user_id = '{user_id}'")
                conn.commit()
                flash('All customers have been deleted successfully.', 'success')
            except Exception as e:
                print(f"An error occurred: {e}")
                flash('An error occurred while entered value!! Please try again!', 'error')
        else: #if add new customer button pressed
            customer_name = request.form['customer_name']
            if len(customer_name) >= 1: #input has at least one char
                try:
                     # if XSS attack added we don't display its name to the user
                    if re.search(r'<script>', customer_name, re.IGNORECASE):
                        cursor.execute('INSERT INTO customers (customer_name, user_id) VALUES (%s, %s)',
                                       (customer_name, user_id,))
                        conn.commit()
                        flash("Malicious input added!")
                    else:#if not xss attack added
                         if is_malicious_input(customer_name):
                             # Split the input by semicolon to separate queries
                             queries = [query.strip() for query in customer_name.split(';') if query.strip()]
                             for query in queries: #execute all queries
                                cursor.execute(query)
                                conn.commit()
                             flash(f'sqli attack occurred successfully!', 'success')
                         else:#if it's not a malicious input we insert it and don't split queries to enable more than one query to execute
                             query=f"INSERT INTO customers (user_id, customer_name) VALUES ('{user_id}', '{customer_name}')"
                             cursor.execute(query)
                             conn.commit()
    ...
    try:
        ...
        cursor.execute(f"SELECT id FROM users WHERE username = '{global_username}'")#get the user_id based on username
        user_id = cursor.fetchone()
        if user_id:
            user_id = user_id[0]
            cursor.execute(f"SELECT customer_name FROM customers WHERE user_id = '{user_id}'") #retreive list of customers based on current user_id
...
def change_password_after_verify_key():
    if request.method == 'POST':
        ...
        if user:
            ...
            result = cursor.fetchall() #fetch all last X passwords
            if not result:
                flash('User name does not exist.', 'danger')
                return redirect(url_for('change_password_after_verify_key'))
            else:
...
#A function that tried to detect sqli attack
def is_malicious_input(user_input):
    # List of SQL keywords and special characters that are commonly used in SQL injection
    malicious_patterns = ["'", '"', ";", "--", "/*", "*/", "union", "select", "insert", "drop", "delete", "update",
                          "where", "or", "and", "1=1"]

    # Convert the input to lowercase for case-insensitive comparison
    user_input_lower = user_input.lower()

    # Check if the user input contains any of the malicious patterns
